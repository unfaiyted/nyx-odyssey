...
import { db } from '../../db';
import { trips, itineraryItems, nutritionEntries, packingItems, flights, accommodations } from '../../db/schema';
...
export const APIRoute = createAPIFileRoute('/api/notifications')({
  GET: async () => {
      ...
      const upcoming = await db.select().from(itineraryItems)
        .where(
          and(
            ...
            lte(itineraryItems.date, threeDays),
            eq(itineraryItems.completed, false),
          )
        )
    ...
    // 2. Overdue tasks (past itinerary items not completed)
    for (const tripId of activeTripIds) {
      const overdue = await db.select().from(itineraryItems)
        .where(
          and(
            eq(itineraryItems.tripId, tripId),
            lt(itineraryItems.date, today),
            eq(itineraryItems.completed, false),
          )
        )
        .orderBy(desc(itineraryItems.date))
        .limit(5);
      ...
      for (const item of overdue) {
        notifications.push({
          ...
          severity: 'urgent',
          title: item.title,
          description: `Overdue since ${item.date}`,
          timestamp: item.date,
          tripId,
        });
      }
    ...
    // 3. Upcoming flights (within 7 days)
    ...
    for (const tripId of activeTripIds) {
      const upcomingFlights = await db.select().from(flights)
        .where(
          and(
            ...
            gte(flights.departureDate, today),
            lte(flights.departureDate, sevenDays),
          )
        )
        .orderBy(asc(flights.departureDate))
        .limit(3);
      ...
      for (const flight of upcomingFlights) {
        ...
        notifications.push({
          ...
          timestamp: flight.departureDate,
          tripId,
        });
      }
    }

    // 4. Missed nutrition goals (check if yesterday had < 1200 calories or < 80g protein)
    const yesterdayNutrition = await db.select().from(nutritionEntries)
      .where(eq(nutritionEntries.date, yesterday));

    if (yesterdayNutrition.length > 0) {
      const totals = yesterdayNutrition.reduce(
        (acc, e) => ({
      ...
      if (totals.calories < 1200) {
        notifications.push({
          id: `nutrition-cal-${yesterday}`,
          type: 'nutrition',
          severity: 'warning',
          title: 'Low calorie intake yesterday',
          description: `${Math.round(totals.calories)} cal — below 1,200 target`,
          timestamp: yesterday,
        });
      }
      if (totals.protein < 80) {
        notifications.push({
          id: `nutrition-pro-${yesterday}`,
          type: 'nutrition',
          severity: 'warning',
          title: 'Low protein yesterday',
          description: `${Math.round(totals.protein)}g — below 80g target`,
          timestamp: yesterday,
        });
      }
    } else {
      // No entries at all yesterday
      notifications.push({
        id: `nutrition-none-${yesterday}`,
        type: 'nutrition',
        severity: 'warning',
        title: 'No nutrition logged yesterday',
        description: 'Remember to track your meals',
        timestamp: yesterday,
      });
    }

    // 5. Unpacked items for trips starting within 7 days
    for (const trip of activeTrips) {
      if (trip.startDate && trip.startDate <= sevenDays && trip.startDate >= today) {
        const unpacked = await db.select().from(packingItems)
          .where(
            and(
              eq(packingItems.tripId, trip.id),
              eq(packingItems.packed, false),
            )
          );
