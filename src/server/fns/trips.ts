...
import { db } from '../../db';
import { trips, itineraryItems, tripDestinations, accommodations, budgetItems, budgetCategories, packingItems, flights, rentalCars, tripRoutes, tripCronJobs, tripRecommendations } from '../../db/schema';
...
export const getTrip = createServerFn({ method: 'GET' })
  .handler(async ({ data }) => {
    ...
    const [itin, dests, accom, budget, budgetCats, packing, flightRows, rentalCarRows, routeRows, cronJobRows, recs] = await Promise.all([
